...
namespace
{
    ...
    auto treasure_map_deadhand_advance_norm(unsigned char ch) -> float
    {
        static const std::unordered_map<unsigned char, float> k_map = {
            {' ', 0.313264f}, {'!', 0.260583f}, {'"', 0.327375f}, {'#', 0.520226f}, {'$', 0.461900f}, {'%', 0.537159f},
            {'&', 0.424271f}, {'\'', 0.163688f}, {'(', 0.342427f}, {')', 0.424271f}, {'*', 0.298213f}, {'+', 0.300094f},
            {',', 0.219191f}, {'-', 0.333960f}, {'.', 0.175917f}, {'/', 0.421449f}, {'0', 0.433678f}, {'1', 0.371590f},
            {'2', 0.427093f}, {'3', 0.456256f}, {'4', 0.450611f}, {'5', 0.538100f}, {'6', 0.501411f}, {'7', 0.508937f},
            {'8', 0.469426f}, {'9', 0.417686f}, {':', 0.254939f}, {';', 0.299153f}, {'<', 0.461900f}, {'=', 0.416745f},
            {'>', 0.483537f}, {'?', 0.337723f}, {'@', 0.620884f}, {'A', 0.894638f}, {'B', 0.603010f}, {'C', 0.542803f},
            {'D', 0.689558f}, {'E', 0.670743f}, {'F', 0.658514f}, {'G', 0.653810f}, {'H', 0.692380f}, {'I', 0.428975f},
            {'J', 0.621825f}, {'K', 0.749765f}, {'L', 0.638758f}, {'M', 1.070555f}, {'N', 0.773283f}, {'O', 0.571966f},
            {'P', 0.549389f}, {'Q', 0.656632f}, {'R', 0.635936f}, {'S', 0.529633f}, {'T', 0.626529f}, {'U', 0.691439f},
            {'V', 0.612418f}, {'W', 0.777987f}, {'X', 0.614299f}, {'Y', 0.542803f}, {'Z', 0.723424f}, {'[', 0.397930f},
            {'\\', 0.317968f}, {']', 0.436500f}, {'^', 0.235183f}, {'_', 0.650988f}, {'`', 0.148636f}, {'a', 0.336783f},
            {'b', 0.304798f}, {'c', 0.322672f}, {'d', 0.469426f}, {'e', 0.286924f}, {'f', 0.399812f}, {'g', 0.454374f},
            {'h', 0.375353f}, {'i', 0.232361f}, {'j', 0.363123f}, {'k', 0.359360f}, {'l', 0.243650f}, {'m', 0.572907f},
            {'n', 0.412041f}, {'o', 0.269991f}, {'p', 0.354657f}, {'q', 0.307620f}, {'r', 0.319849f}, {'s', 0.404516f},
            {'t', 0.284102f}, {'u', 0.391345f}, {'v', 0.431797f}, {'w', 0.469426f}, {'x', 0.465663f}, {'y', 0.362183f},
            {'z', 0.423330f}, {'{', 0.299153f}, {'|', 0.271872f}, {'}', 0.290687f}, {'~', 0.395108f},
        };

        if (ch == '\t')
        {
            return k_map.at(' ') * 4.0f;
        }
        if (ch < 32)
        {
            return 0.0f;
        }
        if (const auto it = k_map.find(ch); it != k_map.end())
        {
            return it->second;
        }
        if (std::isupper(ch) != 0)
    ...
    auto estimate_treasure_map_line_width(std::string_view text, float font, float width_factor) -> float
    {
        const float effective_font = std::max(0.001f, font);
        const float factor = std::clamp(width_factor, 0.20f, 2.00f);
        float sum_norm = 0.0f;
        for (unsigned char ch : text)
        {
            sum_norm += treasure_map_deadhand_advance_norm(ch);
        }
        return std::max(0.0f, sum_norm * effective_font * factor);
    }

    auto fit_text_for_plaque_pak_weighted(
        std::string_view input_text,
        float char_width_factor,
        float font_min,
        float font_max,
        float horizontal_budget) -> AutoSizeResult
    {
        ...
        for (float font = k_font_max; font >= k_font_min; font -= 0.5f)
        {
            ...
            for (const auto& raw_line : input_lines)
            {
                ...
                auto words = split_words(raw_line);
                const bool single_word_line = words.size() == 1;
            ...
            if (!found_fallback ||
                candidate_coverage_chars > best_fallback_coverage ||
                (candidate_coverage_chars == best_fallback_coverage && font > best_fallback_font))
            {
    ...
    auto sync_button_hover_with_normal_style(UObject* button) -> bool
    {
        if (!button || !button->GetClassPrivate())
        {
            return false;
        }

        bool copied = false;
        for_each_property_in_chain_compat(button->GetClassPrivate(), [&](FProperty* prop) {
            if (!prop || copied || prop->GetClass().HashObject() != FStructProperty::StaticClass().HashObject())
            {
                return;
            }
            const auto prop_name = lower_copy_ascii(RC::to_string(prop->GetName()));
            if (prop_name != "widgetstyle" && prop_name != "style")
            {
                return;
            }

            auto* style_prop = static_cast<FStructProperty*>(prop);
            auto* style_storage = style_prop->ContainerPtrToValuePtr<void>(button);
            RC::Unreal::UScriptStruct* style_struct = style_prop->GetStruct().Get();
            if (!style_storage || !style_struct)
            {
                return;
            }

            void* normal_storage = nullptr;
            void* hovered_storage = nullptr;
            int32_t normal_size = 0;
            int32_t hovered_size = 0;

            for_each_property_in_chain_compat(style_struct, [&](FProperty* inner_prop) {
                if (!inner_prop || inner_prop->GetClass().HashObject() != FStructProperty::StaticClass().HashObject())
                {
                    return;
                }
                const auto inner_name = lower_copy_ascii(RC::to_string(inner_prop->GetName()));
                if (inner_name == "normal")
                {
                    normal_storage = inner_prop->ContainerPtrToValuePtr<void>(style_storage);
                    normal_size = inner_prop->GetSize();
                }
                else if (inner_name == "hovered")
                {
                    hovered_storage = inner_prop->ContainerPtrToValuePtr<void>(style_storage);
                    hovered_size = inner_prop->GetSize();
                }
            });

            if (normal_storage && hovered_storage && normal_size > 0 && normal_size == hovered_size)
            {
                std::memcpy(hovered_storage, normal_storage, static_cast<size_t>(normal_size));
                copied = true;
            }
        });
...
namespace WindroseTextSigns
{
    ...
    auto SignTextMod::apply_phase7_umg_debug_scales(const char* reason) -> void
    {
        ...
        m_phase7_debug_refresh_label_offset_x = std::clamp(m_phase7_debug_refresh_label_offset_x, -60.0f, 60.0f);
        m_phase7_debug_refresh_label_offset_y = std::clamp(m_phase7_debug_refresh_label_offset_y, -60.0f, 60.0f);
        bool refresh_label_offset_ok = false;
        if (m_phase7_umg_refresh_all_label && is_uobject_reflection_safe(m_phase7_umg_refresh_all_label))
        {
            refresh_label_offset_ok = invoke_set_vector2d_value(
                m_phase7_umg_refresh_all_label,
                STR("SetRenderTranslation"),
                STR("/Script/UMG.Widget:SetRenderTranslation"),
                m_phase7_debug_refresh_label_offset_x,
                m_phase7_debug_refresh_label_offset_y);
        }
        ...
        if (reason)
        {
            ...
            std::ostringstream refresh_x{};
            refresh_x << std::fixed << std::setprecision(2) << m_phase7_debug_refresh_label_offset_x;
            std::ostringstream refresh_y{};
            refresh_y << std::fixed << std::setprecision(2) << m_phase7_debug_refresh_label_offset_y;
            log_line("[phase7-umg] debug_render_scale_apply reason=" + std::string{reason} +
                     " hint=" + hint_text.str() +
                     " status=" + status_text.str() +
                     " refreshTextOffsetX=" + refresh_x.str() +
                     " refreshTextOffsetY=" + refresh_y.str() +
                     " hintOk=" + std::string{hint_ok ? "true" : "false"} +
                     " statusOk=" + std::string{status_ok ? "true" : "false"} +
                     " refreshTextOffsetOk=" + std::string{refresh_label_offset_ok ? "true" : "false"});
        }
    }
    ...
    auto SignTextMod::invalidate_phase7_umg_widget_cache(const std::string& reason) -> void
    {
        ...
        m_phase7_umg_refresh_all_label = nullptr;
    ...
    auto SignTextMod::ensure_phase7_umg_widget_built() -> bool
    {
        ...
        const bool refresh_hover_matches_normal = !refresh_all_button || sync_button_hover_with_normal_style(refresh_all_button);
        ...
        m_phase7_umg_refresh_all_label = refresh_all_label;
        m_phase7_umg_refresh_all_was_pressed = false;
        m_phase7_umg_refresh_all_clicked_once = false;
        cache_phase7_umg_function_pointers();
        apply_phase7_umg_debug_scales("prewarm_init");

        // Build-only here; session-ready prewarm attaches the cached widget collapsed.
        m_phase7_umg_in_viewport = false;
        log_line(std::string{"[phase7-umg] prewarm_result built=true added=false collapsed=true focus=false"} +
                 " style=" + std::string{(title_text && hint_text && status_text && refresh_button_text && input_text && title_color && hint_color && status_color && refresh_label_color && input_color &&
                                         ...
                                         refresh_hover_matches_normal) ? "true" : "false"});
        return true;
    }
    ...
    auto SignTextMod::maybe_prewarm_phase7_umg_editor() -> void
    {
        ...
        if (built && m_phase7_umg_widget)
        {
            ...
            cache_phase7_umg_function_pointers();
            mark_phase7_status_dirty("prewarm_editor");
            refresh_phase7_umg_status(true, "prewarm_editor");
            apply_phase7_umg_debug_scales("prewarm_editor");
    ...
    auto SignTextMod::open_phase7_umg_editor_for_selection() -> bool
    {
        ...
        const bool widget_ready =
            m_phase7_umg_widget &&
            m_phase7_umg_text_box &&
            is_uobject_reflection_safe(m_phase7_umg_widget) &&
            is_uobject_reflection_safe(m_phase7_umg_text_box);
        const bool widget_built = widget_ready
            ? true
            : budgeted_step("ensure_phase7_umg_widget_built", [&]() {
        ...
        const bool prewarmed_attached =
            m_phase7_umg_prewarm_succeeded &&
            m_phase7_umg_in_viewport &&
            !m_phase7_last_close_removed;
        const bool collapsed = prewarmed_attached
            ? true
            : budgeted_step("open_collapse", [&]() {
                return invoke_phase7_set_visibility(1, "open_collapse");
            });
        const bool input_mode_pre = m_phase7_ui_input_mode_active
            ? budgeted_step("set_input_mode_pre_cached", [&]() {
                install_phase7_keyboard_capture_hook();
                m_phase7_keyboard_capture_active.store(true);
                return true;
            })
            : budgeted_step("set_input_mode_pre", [&]() {
        ...
        log_line("[phase7-umg] open_result added=" + std::string{added ? "true" : "false"} +
                 ...
                 " prewarmedFastPath=" + std::string{prewarmed_attached ? "true" : "false"} +
        ...
        if (type == "route_probe_ack")
        {
                ...
                !m_bridge_route_lock_acquired)
            {
                ...
                log_line("[bridge-route] route_probe_result candidate=" + result_candidate +
                         ":" + std::to_string(m_bridge_udp_port) +
                         " result=success");
                std::string lock_host = result_candidate;
    ...
    auto SignTextMod::tick_bridge_route_discovery() -> void
    {
        ...
        auto handle_probe_exhausted = [&](const std::string& details) {
            ...
            const bool repeated_post_ready_failures = m_bridge_route_post_ready_retry_stage >= 3;
            if (!m_bridge_route_retry_consumed &&
                repeated_post_ready_failures &&
                !m_route_lock_failed_no_reachable_endpoint)
            {
                m_bridge_route_retry_consumed = true;
                m_route_lock_failed_no_reachable_endpoint = true;
                log_line("[bridge-route] route_lock_failed_no_reachable_endpoint details=post_ready_probe_cycles_exhausted");
                mark_phase7_status_dirty("route_lock_failed_no_reachable_endpoint");
            }
            else if (!m_route_lock_failed_no_reachable_endpoint)
            {
                const bool should_log_retrying =
                    m_bridge_route_wait_last_reason != details ||
                    m_bridge_route_wait_last_log.time_since_epoch().count() == 0 ||
                    (now - m_bridge_route_wait_last_log) >= std::chrono::seconds(3);
                if (should_log_retrying)
                {
                    log_line("[bridge-route] route_lock_pending reason=post_ready_probe_retry details=" + details);
                    m_bridge_route_wait_last_reason = details;
                    m_bridge_route_wait_last_log = now;
                }
            }
        ...
        {
            if (m_session_ready_latched &&
                m_bridge_route_probe_index >= m_bridge_route_probe_candidates.size())
            {
                log_line("[bridge-route] route_probe_cycle_result result=timeout candidates=" +
                         std::to_string(m_bridge_route_probe_candidates.size()) +
                         " epoch=" + std::to_string(m_session_epoch));
                m_bridge_route_probe_waiting_ack = false;
                m_bridge_route_probe_active = false;
    ...
    auto SignTextMod::start_next_route_probe(const std::chrono::steady_clock::time_point now) -> bool
    {
        ...
        if (m_session_ready_latched)
        {
            ...
            const size_t stage_index = static_cast<size_t>(std::min<uint32_t>(
                m_bridge_route_post_ready_retry_stage,
                static_cast<uint32_t>(k_post_ready_retry_delays.size() - 1)));
            const auto probe_wait_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(k_post_ready_retry_delays[stage_index]);
            const auto token_base = std::to_string(m_session_epoch) + "-" +
            ...
            for (size_t i = 0; i < m_bridge_route_probe_candidates.size(); ++i)
            {
                const auto& next = m_bridge_route_probe_candidates[i];
                const auto& host = next.first;
                const auto& source = next.second;
                const auto token = token_base + "-" + std::to_string(i);
                ...
                sent_count += sent ? 1u : 0u;
                failed_count += sent ? 0u : 1u;
            ...
            m_bridge_route_probe_token = token_base;
            m_bridge_route_probe_waiting_ack = any_sent;
            m_bridge_route_probe_active = any_sent;
            m_bridge_route_probe_deadline = now + probe_wait_ms;
            log_line("[bridge-route] route_probe_cycle_start epoch=" + std::to_string(m_session_epoch) +
                     " candidates=" + std::to_string(m_bridge_route_probe_candidates.size()) +
                     " sent=" + std::to_string(sent_count) +
                     " failed=" + std::to_string(failed_count) +
                     " waitMs=" + std::to_string(static_cast<long long>(probe_wait_ms.count())) +
                     " stage=" + std::to_string(m_bridge_route_post_ready_retry_stage));
            return any_sent;
        }
    ...
    auto SignTextMod::apply_text_to_actor_component(AActor* actor, const std::string& text_value) -> bool
    {
        ...
        AutoSizeResult runtime_fit = fonted
            ? fit_text_for_plaque_pak_weighted(
                  normalized_for_fit,
                  m_autosize_char_width_factor,
                  font_min,
                  font_max,
                  m_autosize_horizontal_budget)
            : fit_text_for_plaque(
                  normalized_for_fit,
                  m_autosize_char_width_factor,
                  font_min,
        ...
        log_line("[autosize-widthmap] key=" + key +
                 " source=" + std::string{fonted ? "pak" : "fallback"} +
                 " rows=" + std::to_string(runtime_fit.rows) +
                 " fontSize=" + std::to_string(desired_font_size) +
                 " budget=" + std::to_string(m_autosize_horizontal_budget));
    ...
    auto SignTextMod::tick_r5_readiness_markers() -> void
    {
        ...
        constexpr uintmax_t k_r5_bootstrap_max_bytes_per_tick = 16ull * 1024ull * 1024ull;
        constexpr auto k_r5_bootstrap_time_budget = std::chrono::milliseconds(20);
        const bool role_pending_in_window = m_session_window_open && !m_role_lock_acquired;
        const bool ready_pending_after_role = m_role_lock_acquired && !m_session_ready_latched;
        const bool route_pending_remote =
            m_bridge_role == BridgeRole::RemoteClient &&
            (!m_bridge_route_gate_open || !m_bridge_route_lock_acquired);
        const bool bootstrap_catchup_mode =
            role_pending_in_window ||
            ready_pending_after_role ||
            route_pending_remote ||
            m_server_role_classification_pending;
        const int max_chunks_this_tick = bootstrap_catchup_mode
            ? static_cast<int>(k_r5_bootstrap_max_bytes_per_tick / k_r5_read_chunk_bytes)
            : k_r5_max_chunks_per_tick;
        const auto parse_budget_deadline = std::chrono::steady_clock::now() + k_r5_bootstrap_time_budget;
        bool backlog_logged_this_tick = false;
        int chunks_processed = 0;
        uintmax_t bytes_processed = 0;
        bool stopped_for_time_budget = false;
        bool stopped_for_byte_budget = false;
        ...
        while (m_destroy_signal_log_offset < size && chunks_processed < max_chunks_this_tick)
        {
            if (bootstrap_catchup_mode &&
                bytes_processed >= k_r5_bootstrap_max_bytes_per_tick)
            {
            ...
            if (bootstrap_catchup_mode &&
                chunks_processed > 0 &&
                std::chrono::steady_clock::now() >= parse_budget_deadline)
            {
            ...
            if (!backlog_logged_this_tick && remaining_bytes > k_r5_read_chunk_bytes)
            {
                ...
                log_line("[save] r5log_backlog_detected bytes=" +
                         ...
                         " maxChunksThisTick=" + std::to_string(max_chunks_this_tick) +
                         " mode=" + std::string{bootstrap_catchup_mode ? "bootstrap_catchup" : "normal"});
                backlog_logged_this_tick = true;
            }
            input.clear();
            input.seekg(static_cast<std::streamoff>(chunk_start_offset), std::ios::beg);
            const auto remaining_budget = bootstrap_catchup_mode
                ? (bytes_processed >= k_r5_bootstrap_max_bytes_per_tick
                    ? static_cast<uintmax_t>(0)
                    : k_r5_bootstrap_max_bytes_per_tick - bytes_processed)
                : k_r5_read_chunk_bytes;
            const auto bytes_to_read = std::min<uintmax_t>(
                remaining_bytes,
                std::min<uintmax_t>(k_r5_read_chunk_bytes, remaining_budget));
            if (bytes_to_read == 0)
            {
                stopped_for_byte_budget = bootstrap_catchup_mode;
                break;
            }
            std::string chunk{};
            ...
            ++chunks_processed;
            bytes_processed += static_cast<uintmax_t>(chunk.size());
            if (server_runtime_process)
            {
        ...
        if (m_destroy_signal_log_offset < size &&
            (chunks_processed >= max_chunks_this_tick || stopped_for_time_budget || stopped_for_byte_budget))
        {
            ...
            log_line("[save] r5log_backlog_catchup_deferred remainingBytes=" +
                     ...
                     " bytesProcessed=" + std::to_string(static_cast<unsigned long long>(bytes_processed)) +
                     " maxChunksThisTick=" + std::to_string(max_chunks_this_tick) +
                     " mode=" + std::string{bootstrap_catchup_mode ? "bootstrap_catchup" : "normal"} +
                     " stoppedForTimeBudget=" + std::string{stopped_for_time_budget ? "true" : "false"} +
                     " stoppedForByteBudget=" + std::string{stopped_for_byte_budget ? "true" : "false"});
        }
        if (server_runtime_process)
        {
    ...
    auto SignTextMod::render_ui() -> void
    {
        ...
        float refresh_text_offset_x = m_phase7_debug_refresh_label_offset_x;
        float refresh_text_offset_y = m_phase7_debug_refresh_label_offset_y;
        ...
        bool refresh_text_x_changed = ImGui::DragFloat("Refresh Text Align X", &refresh_text_offset_x, 0.25f, -60.0f, 60.0f, "%.2f");
        bool refresh_text_y_changed = ImGui::DragFloat("Refresh Text Align Y", &refresh_text_offset_y, 0.25f, -60.0f, 60.0f, "%.2f");
        if (status_scale_changed || hint_scale_changed || refresh_text_x_changed || refresh_text_y_changed)
        {
            ...
            m_phase7_debug_refresh_label_offset_x = std::clamp(refresh_text_offset_x, -60.0f, 60.0f);
            m_phase7_debug_refresh_label_offset_y = std::clamp(refresh_text_offset_y, -60.0f, 60.0f);
        ...
        if (auto found = m_labels.find(key); found != m_labels.end())
        {
            ...
            float autosize_horizontal_budget_value = std::clamp(m_autosize_horizontal_budget, 80.0f, 220.0f);
            ...
            if (ImGui::DragFloat("Autosize Horizontal Budget", &autosize_horizontal_budget_value, 1.0f, 80.0f, 220.0f, "%.0f"))
            {
                m_autosize_horizontal_budget = std::clamp(autosize_horizontal_budget_value, 80.0f, 220.0f);
                log_line("[autosize] horizontal_budget_updated value=" + std::to_string(m_autosize_horizontal_budget));
                if (live_surface_tune)
                {
                    const bool rendered = apply_text_to_actor_component(m_selected->actor, found->second.text);
                    log_line("[phase4] autosize_budget_tune_live key=" + key +
                             " rendered=" + std::string{rendered ? "true" : "false"});
                }
            }
